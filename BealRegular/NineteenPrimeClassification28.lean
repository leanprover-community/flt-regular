...
/-! Exhaustive p = 19 split-prime classification for k in [3500, 3624]. -/

set_option maxHeartbeats 0 in
-- Checks every candidate in this bounded interval by explicit arithmetic cases.
set_option maxRecDepth 20000 in
theorem prime_mem_exceptionalSplitPrimesNineteen_28 :
    ∀ k ∈ Finset.Icc 3500 3624, (19 * k + 1).Prime →
      19 * k + 1 ∈ exceptionalSplitPrimesNineteen_14 := by
  intro k hk hp
  simp only [Finset.mem_Icc] at hk
  obtain ⟨hk3500, hk3624⟩ := hk
  interval_cases k
  all_goals solve
    | norm_num at hp
