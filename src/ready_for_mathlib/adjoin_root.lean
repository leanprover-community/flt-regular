...
begin
  ...
  replace hP₁ := eq_zero_of_ne_zero_of_mul_left_eq_zero ((map_ne_zero_iff _
    (no_zero_smul_divisors.algebra_map_injective R A)).2 hPcont) hP₁,
...
power_basis.map (adjoin_root.power_basis' (minpoly.monic hx)) (minpoly.to_adjoin_equiv hx)
