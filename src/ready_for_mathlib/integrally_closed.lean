...
begin
  ...
  replace hdiv : ∀ n < P₁.nat_degree, algebra_map R S p ∣ P₁.coeff n,
  { intros i hi,
    obtain ⟨r, hr⟩ := hdiv i (lt_of_lt_of_le hi (nat_degree_map_le _ _)),
    rw [coeff_map, hr, ring_hom.map_mul],
    exact dvd_mul_right _ _ },
  intros i hi,
  exact eisenstein hx ((monic_map (algebra_map R S) hmo))
    hdiv i (le_trans (nat_degree_map_le _ _) hi)
end
...
begin
  choose! f hf using eisenstein_aeval hroot hmo hdiv,
  ...
  simp_rw [smul_mul_assoc, add_mul, smul_mul_assoc, one_mul, sum_mul, sum_congr rfl this,
    algebra.smul_mul_assoc, ← smul_def, smul_smul, mul_comm _ p, ← smul_smul] at hQ,
