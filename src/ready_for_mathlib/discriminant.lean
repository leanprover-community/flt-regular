...
begin
  ...
  { rw [← basis.sum_repr pb.basis z, coe_sum, finset.smul_sum],
    ...
    replace this := this i,
    rw [hA, ← discr_def, pi.smul_apply, mem_bot] at this,
    obtain ⟨r, hr⟩ := this,
  ...
  have cramer := mul_vec_cramer A b,
  rw [← H, ← mul_vec_smul] at cramer,
  replace cramer := congr_arg (mul_vec A⁻¹) cramer,
  rw [mul_vec_mul_vec, nonsing_inv_mul _ hinv, ← hA, mul_vec_mul_vec, nonsing_inv_mul _ hinv,
