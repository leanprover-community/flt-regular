...
def flt_ideals (x y i : ℤ) : ideal RR :=
  ideal.span ({ x + y * ((zeta_runity n (𝓞 ℚ) RR) ^ i : units RR) } : set RR)
...
lemma mem_flt_ideals {x y i : ℤ} :
  (x : RR) + y * ((zeta_runity n (𝓞 ℚ) RR) ^ i : units RR) ∈ flt_ideals n x y i :=
...
lemma add_eq_mul_one_add_div {a : units K} {b : K} : ↑a + b = a * (1 + ↑a⁻¹ * b) :=
...
begin
  ...
  have : ∃ v : units RR, (v : RR) * y * (1 - (zeta_runity n (𝓞 ℚ) RR)) ∈ I,
