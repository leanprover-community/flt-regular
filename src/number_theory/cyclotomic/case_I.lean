...
lemma exists_int_sum_eq_zero (hpodd : p ≠ 2) [hp : fact(p : ℕ).prime] {x y i : ℤ} {u : (𝓞 K)ˣ}
  ...
  ∃ k : ℤ, (x : 𝓞 K) + y * (hζ.unit' ^ i : (𝓞 K)ˣ) - (hζ.unit' ^ (2 * k) : (𝓞 K)ˣ) *
    (x + y * (hζ.unit' ^ -i : (𝓞 K)ˣ)) ∈ span ({p} : set (𝓞 K)) :=
begin
  ...
  { --using the new typeclasses for morphisms this will be easier
    change (_ : K) = (gal_conj K p).to_alg_hom.to_ring_hom _,
    rw [ring_hom.map_add, ring_hom.map_int_cast, ring_hom.map_mul, ring_hom.map_int_cast],
    simp only [zpow_neg, coe_coe, alg_equiv.to_alg_hom_eq_coe, alg_hom.to_ring_hom_eq_coe,
      hζ.coe_unit'_coe, add_right_inj],
    rw [ring_hom.map_zpow],
    simp only [coe_zpow, coe_coe, hζ.coe_unit'_coe, alg_equiv.to_alg_hom_eq_coe,
      alg_hom.to_ring_hom_eq_coe, alg_hom.coe_to_ring_hom, alg_equiv.coe_alg_hom],
    rw [gal_conj_zeta_runity hζ],
    simp [← coe_life] },
  ...
  change _ = (gal_conj K p).to_alg_hom.to_ring_hom _ at this,
  rw [h', sub_eq_iff_eq_add.1 hγ', H, mul_mem_class.coe_mul, ring_hom.map_mul, ring_hom.map_mul,
    ring_hom.map_add, ring_hom.map_int_cast, ring_hom.map_mul, ← coe_coe β, coe_zpow,
    ring_hom.map_zpow, coe_coe, coe_zpow] at this,
  simp only [coe_coe, hζ.coe_unit'_coe, alg_equiv.to_alg_hom_eq_coe,
    alg_hom.to_ring_hom_eq_coe, alg_hom.coe_to_ring_hom, alg_equiv.coe_alg_hom,
    subring_class.coe_nat_cast, map_nat_cast] at this,
