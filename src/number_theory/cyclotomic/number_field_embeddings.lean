...
lemma mem_root_set_map_of_injective [comm_ring S] [is_domain S] {f : R →+* S} {x : S}
  (hf : function.injective f) (hp : p ≠ 0) : x ∈ (p.map f).root_set S ↔ p.eval₂ f x = 0 :=
begin
  suffices : map f p ≠ 0,
  { rw mem_root_set_iff,
    simp only [mem_root_set_iff, aeval_def, ←eval_map, algebra.id.map_eq_id, map_id],
    exact this, },
  contrapose! hp,
  simp only [polynomial.ext_iff, coeff_map, coeff_zero] at hp ⊢,
  exact λ n : ℕ, hf (by rw [(hp n), f.map_zero]),
end
