...
! This file was ported from Lean 3 source module flt_three.spts
-/
...
theorem Zsqrtd.exists {d : ℤ} (a : ℤ√d) (him : a.im ≠ 0) :
  ...
  · use a; simp only [hre.le, him, eq_self_iff_true, and_self_iff, Ne.def, not_false_iff]
...
theorem Spts.mul_of_dvd' {a p : ℤ√(-3)} (hdvd : p.norm ∣ a.norm) (hpprime : Prime p.norm) :
  ...
  obtain ⟨A, HA⟩ : ∃ A : Units ℤ, p.norm ∣ p.re * a.im + A * a.re * p.im := by
    cases' h0 with h0 h0 <;> [use -1; use 1] <;> convert h0 <;> simp
  ...
  · set X : ℤ√(-3) := ⟨p.re * a.re - A * 3 * p.im * a.im, p.re * a.im + A * a.re * p.im⟩ with HX
    obtain ⟨U, HU⟩ : (p.norm : ℤ√(-3)) ∣ X :=
      ...
      have : X.re ^ 2 = X.norm - 3 * X.im ^ 2 := by rw [Zsqrtd.norm_def]; ring
      rw [this]
      apply dvd_sub
      · use a.norm
        trans
    ...
    have : p.norm = p.re ^ 2 + 3 * ↑A ^ 2 * p.im ^ 2 := by rw [Zsqrtd.norm_def, HAsq]; ring
...
theorem factors' (a : ℤ√(-3)) (f : ℤ) (g : ℤ) (hodd : Odd f) (hgpos : g ≠ 0)
  ...
  · obtain ⟨c, hc⟩ := factors2 H
    ...
    · rw [Int.natAbs_mul]
      apply lt_mul_of_one_lt_left (Int.natAbs_pos_of_ne_zero hg'pos)
      norm_num
...
theorem Zqrtd.factor_div (a : ℤ√(-3)) {x : ℤ} (hodd : Odd x) :
  ...
  · rw [← mul_lt_mul_left (by norm_num : (0 : ℤ) < 4)]
    calc
      4 * c'.norm = (2 * c) ^ 2 + 3 * (2 * d) ^ 2 := by rw [Zsqrtd.norm_def]; ring
...
theorem factors (a : ℤ√(-3)) (x : ℤ) (hcoprime : IsCoprime a.re a.im) (hodd : Odd x)
  ...
  have h5 : x * y = g ^ 2 * C'.norm := by
    rw [← hy, HC', Zsqrtd.norm_mul, Zsqrtd.norm_int_cast, ← pow_two]
  obtain ⟨z, hz⟩ : (g ^ 2 : ℤ) ∣ y :=
    by
    have : (g ^ 2 : ℤ) ∣ x * y := by rw [h5]; exact dvd_mul_right _ _
    ...
    simpa only [Zsqrtd.coe_int_re, isCoprime_zero_right, Zsqrtd.coe_int_im, hpprime.not_unit] using
      this
  have h6 : x * z = C'.norm :=
    by
  ...
  ·
    calc
      w.nat_abs ≤ z.nat_abs :=
        Nat.le_of_dvd (Int.natAbs_pos_of_ne_zero h8) (int.nat_abs_dvd_iff_dvd.mpr hwdvd)
      _ ≤ y.nat_abs := by rw [hz, Int.natAbs_mul]; exact Nat.le_mul_of_pos_left (pow_pos hgpos 2)
      _ < x.nat_abs := h3
  · rw [← h6]
...
theorem Spts.pos_of_coprime' {a : ℤ√(-3)} (hcoprime : IsCoprime a.re a.im) : 0 < a.norm :=
  ...
  · symm; exact Spts.ne_zero_of_coprime' _ hcoprime
  ...
  have hq : p.im ^ 2 = 1 := by
    ...
    · contrapose! hfour with hq'
      ...
      calc
        ...
        _ = p.norm := by rw [Zsqrtd.norm_def]; ring
    · rw [← Int.sub_one_lt_iff, sub_self]
      exact sq_pos_of_ne_zero _ hq
  refine' ⟨_, hq⟩
  calc
    p.re ^ 2 = p.re ^ 2 + 3 * p.im ^ 2 - 3 := by rw [hq, mul_one, add_sub_cancel]
    _ = p.norm - 3 := by rw [Zsqrtd.norm_def]; ring
    _ = 1 := by rw [hfour]; norm_num
