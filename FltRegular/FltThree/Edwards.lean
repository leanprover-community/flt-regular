...
! This file was ported from Lean 3 source module flt_three.edwards
-/
...
theorem Zsqrtd.associated_norm_of_associated {d : ℤ} {f g : ℤ√d} (h : Associated f g) :
  ...
  obtain ⟨u, rfl⟩ := h
  have := (Zsqrtd.isUnit_iff_norm_isUnit ↑u).mp u.is_unit
  rw [Zsqrtd.norm_mul]
...
theorem Zsqrt3.coe_of_isUnit {z : ℤ√(-3)} (h : IsUnit z) : ∃ u : Units ℤ, z = u :=
  ...
  use v
  rw [Zsqrtd.ext, hu, ← hv]
  simp only [Zsqrtd.coe_int_re, and_true_iff, eq_self_iff_true, coe_coe, Zsqrtd.coe_int_im]

theorem Zsqrt3.coe_of_is_unit' {z : ℤ√(-3)} (h : IsUnit z) : ∃ u : ℤ, z = u ∧ abs u = 1 :=
  by
...
theorem OddPrimeOrFour.factors (a : ℤ√(-3)) (x : ℤ) (hcoprime : IsCoprime a.re a.im)
  ...
  · obtain ⟨c, hc⟩ := factors a x hcoprime hodd hfactor
    ...
    convert hprime.abs
    rwa [Zsqrtd.norm_def, H, MulZeroClass.mul_zero, sub_zero, ← pow_two, eq_comm] at hc 
...
theorem step1' {a : ℤ√(-3)} (hcoprime : IsCoprime a.re a.im) (heven : Even a.norm) :
  ...
  · cases hu' <;> · rw [hu', Zsqrtd.norm_mul]; congr
...
theorem step1'' {a p : ℤ√(-3)} (hcoprime : IsCoprime a.re a.im) (hp : p.norm = 4) (hq : p.im ≠ 0)
  ...
  · rw [← Int.isUnit_iff_abs_eq] at hp' 
    rwa [Zsqrtd.smul_re, Zsqrtd.smul_im, isCoprime_mul_unit_left hp']
    apply Zsqrtd.coprime_of_dvd_coprime hcoprime
    obtain rfl | rfl := h2 <;> apply dvd_mul_left
  · rw [abs_eq <| zero_le_one' ℤ] at hp' hq' 
    cases hp'
    · have h4 : p = q ∨ p = star q := by
        apply Or.imp _ _ hq' <;> · intro h5; simp [Zsqrtd.ext, hp', h5]
...
theorem OddPrimeOrFour.factors' {a : ℤ√(-3)} (h2 : a.norm ≠ 1) (hcoprime : IsCoprime a.re a.im) :
  ...
  cases huv <;> [use q'; use star q'] <;>
        simp only [IsCoprime, Zsqrtd.star_re, Zsqrtd.star_im, Ne.def, neg_eq_zero,
          Zsqrtd.norm_conj] <;>
    ...
    ∀ n : ℕ,
      a.norm.nat_abs = n →
        ∃ f : Multiset (ℤ√(-3)),
          (a = f.Prod ∨ a = -f.Prod) ∧
            ∀ pq : ℤ√(-3), pq ∈ f → 0 ≤ pq.re ∧ pq.im ≠ 0 ∧ OddPrimeOrFour pq.norm
    by exact this a.norm.nat_abs rfl
  intro n hn
...
theorem associated'_of_abs_eq {x y : ℤ√(-3)} (hre : abs x.re = abs y.re)
  ...
  cases' hre with h1 h1 <;> cases' him with h2 h2 <;> [· left; use 1; · right; use 1;
...
theorem factors_2_even' {a : ℤ√(-3)} (hcoprime : IsCoprime a.re a.im) :
  ...
  · obtain ⟨u, huvcoprime, huvprod⟩ := step1' hcoprime hparity
    ...
    rw [← hn, huvprod, Int.natAbs_mul, lt_mul_iff_one_lt_left (Int.natAbs_pos_of_ne_zero huv)]
    norm_num
  · convert even_zero
...
theorem step5'
  ...
  calc
    Multiset.count x f = Multiset.card (Multiset.filter (Eq x) f) := by
      rw [Multiset.count, Multiset.countp_eq_card_filter]
    _ = Multiset.card (Multiset.filter (fun a : ℤ√(-3) => Eq x.norm a.norm) f) :=
      (congr_arg _ (Multiset.filter_congr fun A HA => _))
    _ = Multiset.countp (Eq x.norm) (Multiset.map Zsqrtd.norm f) :=
      (Multiset.countp_map Zsqrtd.norm f (Eq x.norm)).symm
    _ = Multiset.countp (Eq x.norm) (factorsOddPrimeOrFour a.norm) := _
...
theorem step6 (a b r : ℤ) (hcoprime : IsCoprime a b) (hcube : r ^ 3 = a ^ 2 + 3 * b ^ 2) :
  ...
  by
  set A : ℤ√(-3) := ⟨a, b⟩
  have hcube' : r ^ 3 = A.norm := by simp only [hcube, Zsqrtd.norm_def, A]; ring
