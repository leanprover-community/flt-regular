...
theorem aux_cong0k₁ {k : Fin p} (hcong : k ≡ -1 [ZMOD p]) : k = ⟨p.pred, pred_lt hpri.NeZero⟩ :=
  by
  refine' Fin.ext _
...
theorem auxf0k₁ (hp5 : 5 ≤ p) (b : ℤ) : ∃ i : Fin P, f0k₁ b p (i : ℕ) = 0 :=
  by
...
theorem aux0k₁ {a b c : ℤ} {ζ : R} (hp5 : 5 ≤ p) (hζ : IsPrimitiveRoot ζ p)
  ...
  simpa [f0k₁] using dvd_coeff_cycl_integer hζ (auxf0k₁ hpri hp5 b) key ⟨1, hpri.one_lt⟩
...
theorem auxk₁k₂ {k₁ k₂ : Fin p} (hpri : p.Prime) (hcong : k₂ ≡ k₁ - 1 [ZMOD p]) :
  ...
  rw [show (k₁ : ℤ) = (k₂ : ℕ) by simpa using habs, ← ZMod.int_cast_eq_int_cast_iff, ← sub_eq_zero,
    Int.cast_sub, sub_sub_eq_add_sub, coe_coe, add_sub_cancel', algebraMap.coe_one] at hcong 
