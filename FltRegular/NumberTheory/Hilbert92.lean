...
lemma systemOfUnits.isMaximal [Module.Finite ℤ G] (hf : finrank ℤ G = s * (p - 1))
  ...
  change (Submodule.span A (Set.range sys.units)).toAddSubgroup.FiniteIndex
  rw [AddSubgroup.finiteIndex_iff_finite_quotient]
  change Finite (G ⧸ Submodule.span A (Set.range sys.units))
...
noncomputable def systemOfUnits.index [Module A G] (sys : systemOfUnits p G s) [sys.IsMaximal] :=
  (Submodule.span A (Set.range sys.units)).toAddSubgroup.index
...
instance : CommGroup (RelativeUnits k K) := by
  delta RelativeUnits
  infer_instance
...
lemma unit_to_U_prod {ι} (s : Finset ι) (f : ι → _) :
  ...
  | empty => simp [unit_to_U_one]
  | @insert x s hxs IH =>
    simp [hxs, unit_to_U_mul, IH]
...
lemma finrank_G : finrank ℤ G = (Units.rank k + 1) * (↑p - 1) := by
  ...
  · intros i j e
    exact Additive.toMul.injective <| Units.ext <| Subtype.ext <| (algebraMap k K).injective <|
      congr_arg (fun i : Additive (𝓞 K)ˣ ↦ (↑(↑(Additive.toMul i) : 𝓞 K) : K)) e

theorem Hilbert91 : ∃ S : systemOfUnits p G (NumberField.Units.rank k + 1), S.IsFundamental :=
  systemOfUnits.IsFundamental.existence p hp G (NumberField.Units.rank k + 1)
    (finrank_G p hp hKL σ hσ)

include hp in
lemma h_exists' : ∃ (h : ℕ) (ν : (𝓞 k)ˣ), IsPrimitiveRoot (ν : k) (p ^ h) ∧
...
-- The hypothesis `p ≠ 2` excludes the case where `K = k[i]`.
lemma almostHilbert92 (hpodd : p ≠ 2) : ∃ η : (𝓞 K)ˣ, Algebra.norm k (η : K) = 1 ∧
      ∀ ε : (𝓞 K)ˣ, (η : K) ≠ ε / (σ ε : K) := by
  classical
  ...
  · intro ε hε
    ...
    · by_contra hε'
      ...
      have : i ∉ Set.range Fin.castSucc := by
        rintro ⟨i, rfl⟩
        exact ha' (hε' i)
