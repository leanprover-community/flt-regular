...
-/
def spanIntNorm [Module.Finite R S] (I : Ideal S) : Ideal R :=
...
@[simp]
theorem spanIntNorm_eq_bot_iff [Module.Free R S] [Module.Finite R S]
...
theorem intNorm_mem_spanIntNorm [Module.Finite R S] {I : Ideal S} (x : S) (hx : x ∈ I) :
    Algebra.intNorm R S x ∈ I.spanIntNorm R :=
  subset_span (Set.mem_image_of_mem _ hx)

@[simp]
theorem spanIntNorm_singleton [Module.Finite R S] {r : S} :
...
@[simp]
theorem spanIntNorm_top [Nontrivial S] [Module.Free R S] [Module.Finite R S] :
    spanIntNorm R (⊤ : Ideal S) = ⊤ := by
  rw [← Ideal.span_singleton_one, spanIntNorm_singleton]
  simp

theorem spanIntNorm_mul_spanIntNorm_le [Module.Finite R S] (I J : Ideal S) :
...
However, `Ideal.spanIntNorm_mul_of_field` is harder to apply since we'd need to upgrade a
`CommRing R` instance to a `Field R` instance. -/
theorem spanIntNorm_mul_of_bot_or_top [Module.Free R S]
    [Module.Finite R S] (eq_bot_or_top : ∀ I : Ideal R, I = ⊥ ∨ I = ⊤) (I J : Ideal S) :
