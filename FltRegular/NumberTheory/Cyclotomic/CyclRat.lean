...
noncomputable
def fltIdeals [Fact (p : ℕ).Prime] (x y : ℤ) {η : R} (_ : η ∈ nthRootsFinset p 1) : Ideal R :=
  Ideal.span ({x + η * y} : Set R)

variable {p}

-- why does this not update (n : ℕ+)?
theorem mem_fltIdeals [Fact (p : ℕ).Prime] (x y : ℤ) {η : R} (hη : η ∈ nthRootsFinset p 1) :
...
theorem zeta_sub_one_dvb_p [Fact (p : ℕ).Prime] (ph : 5 ≤ p) {η : R} (hη : η ∈ nthRootsFinset p 1)
    (hne1 : η ≠ 1) : 1 - η ∣ (p : R) := by
...
theorem diff_of_roots2 [Fact (p : ℕ).Prime] (ph : 5 ≤ p) {η₁ η₂ : R} (hη₁ : η₁ ∈ nthRootsFinset p 1)
