...
lemma coeff_minpolyDiv_mem_adjoin : coeff (minpolyDiv R x) i ∈ Algebra.adjoin R {x} := by
  by_contra H
...
lemma span_coeff_minpolyDiv :
  ...
  · rw [← span_range_natDegree_eq_adjoin _ hx, Submodule.span_le]
...
lemma sum_smul_minpolyDiv_eq_X_pow (E) [Field E] [IsAlgClosed E] [Algebra K E]
