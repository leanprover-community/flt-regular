...
\begin{lemma}\label{lem:exists_int_sum_eq_zero}
	Let $p \geq 5$ be an prime number, $\zeta_p$ a $p$-th root of unity and $ \a \in \ZZ[\zeta_p]$. Let $x$ and $y$ be integers such that $x+y\zeta_p^i=u \a^p$ with $u \in \ZZ[\zeta_p]^\times$ and $\a \in \ZZ[\zeta_p]$. Then there is an integer $k$ such that \[x+y\zeta_p^i-\zeta_p^{2k}x-\zeta_p^{2k-i}y \equiv 0 \pmod p.\]
\end{lemma}
\begin{proof}
 \uses{lem:dvd_coeff_cycl_integer,lem:exists_int_sub_pow_prime_dvd}
...
\begin{theorem}\label{theorem:FLT_case_two}\lean{flt_regular_case_two}
	...
	\leanok
	\uses{lemma:may_assume_coprime,defn:is_regular_number,thm:Kummers_lemma}
