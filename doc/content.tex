...
\begin{lemma}\label{lemma:sum_pow_unit}
	\lean{is_primitive_root.sum_pow_unit}
	\leanok
	Let $n$ be a positive integer and let $\zeta_n$ be an $n$-th root of unity. If $k$ is coprime to $n$ then \[1+\zeta_n+\cdots+\zeta_n^{k-1}\] is a unit in $\ZZ[\zeta_n]$ with inverse $\frac{1-\zeta_n}{1-\zeta_n^k}$.
\end{lemma}
\begin{proof}
	\leanok
...
\begin{lemma}\label{lemma:may_assume}
	\uses{lemma:unit_lemma,lemma:sum_pow_unit,lemma:ideals_mult_to_power,theorem:ring_of_ints_of_cyclo}
