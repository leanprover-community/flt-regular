...
\begin{lemma}\label{lem:_lin_indep_iff_disc_ne_zero}
...
\begin{lemma}\label{lem:_disc_change_of_basis}
...
\begin{proof}
	...
	Now, by lemma \ref{lemma:_diff_of_irr_pol} and Proposition \ref{lemma:_alt_definition_of_norm_and_trace} we see that \[N_{K/\QQ}(m_\a'(\a))=\prod_{i=1}^n m_\a'(\a_i)=\prod_{i=1}^n \prod_{i \neq j} (\a_i-\a_j),\] which gives the result.	
\end{proof}	
...
\begin{lemma}\label{lemma:_cyclo_poly}\lean{degree_cyclotomic}
...
\begin{lemma}\label{lemma:_alg_int_abs_val_one}\lean{mem_roots_of_unity_of_abs_eq_one}
...
\begin{proof}
	...
	$(1)$  Reducing modulo $p$, using Fermat's little theorem, you get that if $x \equiv y \equiv -z \pmod p,$ then $3z \equiv 0 \pmod p$. But since $p >3$ this means $p |z$ but this contradicts $\gcd(xyz,p)=1$. Now, if $x \equiv y \pmod p$ then  $x \not \equiv -z \pmod p$ we can relabel $y,z$ so that wlog $x \not \equiv y$ (this uses that $p$ is odd). 
