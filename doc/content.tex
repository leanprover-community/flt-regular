...
\begin{lemma}\label{lemma:disc_via_embs}
    \uses{defn_of_disc, trace_eq_sum_embeddings}
    \leanok
...
\begin{proof}
	...
	Lets now calculate the norm of this: \[N_{K/\QQ} \left(\frac{x_d\a^{n-1}}{p} \right)=\frac{x_d^n N_{K/\QQ}(\a)^{n-1}}{p^n}.\] By Eisenstein the constant coefficient of $m_\a$ is divisible by $p$ but not $p^2$, so since the constant coefficient of $m_\a$ is $N_{K/\QQ}(\a)$ we see that $N_{K/\QQ}(\a)=p a$ where $p \nmid a$. Therefore we have \[N_{K/\QQ} \left(\frac{x_d\a^{n-1}}{p} \right)=\frac{x_d^n p^{n-1}a^{n-1}}{p^n}= \frac{x_d^n a^{n-1}}{p}.\] But this cant be in $\ZZ$ since $p$ doesn't divide $x_d$ or $a$, and this gives us a contradiction since  Proposition \ref{lemma:norm_of_alg_int_is_int} says that the norm of an algebraic integer must be an integer. So $\phi$ couldn't have been an algebraic integer.
...
\begin{lemma}\label{lemma:cyclo_poly}\lean{polynomial.degree_cyclotomic} \leanok
...
\begin{theorem}\label{theorem:ring_of_ints_of_cyclo}
	...
	\lean{is_cyclotomic_extension.rat.is_integral_closure_adjoing_singleton_of_prime_pow}
	\leanok 
