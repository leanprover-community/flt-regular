...
\begin{proof}
	...
	Now, by lemmama \ref{lemma:diff_of_irr_pol} and Proposition \ref{lemma:alt_definition_of_norm_and_trace} we see that \[N_{K/\QQ}(m_\a'(\a))=\prod_{i=1}^n m_\a'(\a_i)=\prod_{i=1}^n \prod_{i \neq j} (\a_i-\a_j),\] which gives the result.	
\end{proof}	
...
\begin{lemma}\label{lemma:cyclo_poly}\lean{polynomial.degree_cyclotomic}\mathlibok
...
\begin{theorem}\label{theorem:FLT_case_one}
	\uses{theorem:FLT_facts,defn:is_regular_number}
